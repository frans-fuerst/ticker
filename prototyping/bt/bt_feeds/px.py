...
from backtrader.utils.py3 import (
    urlopen, urlquote, ProxyHandler, build_opener, install_opener)
...
class PXJsonData(feed.CSVDataBase):
    '''
...
class YahooLegacyCSV(PXJsonData):
    '''
...
class PXCSV(feed.CSVFeedBase):
    DataCls = PXJsonData


class PXData(PXJsonData):
    '''
    ...
    def start(self):
        ...
        super(PXData, self).start()


class PX(feed.CSVFeedBase):
    DataCls = PXData
